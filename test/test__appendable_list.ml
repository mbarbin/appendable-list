...
module Appendable_list = struct
  ...
  module For_testing = struct
    ...
    let map_simple = Appendable_list.For_testing.map_simple
    ...
    let quickcheck_generator =
      ...
      Quickcheck.Generator.weighted_recursive_union
        [ 1., return Appendable_list.empty
        ; 2., return (Appendable_list.singleton Element.S)
        ; ( 1.
          , let%map n = Quickcheck.Generator.small_non_negative_int in
            Appendable_list.of_list
              (Element.L 0 :: Element.L 1 :: List.init n ~f:(fun i -> Element.L (i + 2)))
          )
        ]
  ...
  struct
    ...
    let%test_unit _ =
      let test xss =
        ...
        let ts_right () = List.fold_right xs ~init:empty ~f:(fun t acc -> append t acc) in
        ...
        List.iteri
          ~f:(fun i l ->
            ...
            with
            | exn -> raise_s [%sexp "fail", (i : int), (l : int t), (exn : Exn.t)])
          [ ts_concat; ts_add_front; ts_add_back; ts_left; ts_right; ts_bind; ts_join ]
      in
      List.iter
        ~f:test
        [ []
  ...
  struct
    ...
    let test_sexp_of_t ~sexp_of =
      Quickcheck.test For_testing.quickcheck_generator ~sexp_of ~f:(fun t ->
        [%test_eq: Sexp.t] (sexp_of t) [%sexp (to_list t : For_testing.Element.t list)])
    ;;
    ...
    let test_t_of_sexp ~of_sexp =
      let check sexp =
        [%test_eq: Sexp.t list Or_error.t]
          (Or_error.map (Or_error.try_with (fun () -> of_sexp sexp)) ~f:to_list)
    ...
    let return = return
    let%test_unit "return" = [%test_eq: unit list] (to_list (return ())) (List.return ())
    let map = map
    ...
    let%test_unit "bind" =
      Quickcheck.test
        [%quickcheck.generator: For_testing.t * (For_testing.Element.t -> For_testing.t)]
        ~sexp_of:
          [%sexp_of: For_testing.Element.t t * (For_testing.Element.t -> For_testing.t)]
        ~f:(fun (t, f) ->
          [%test_eq: For_testing.Element.t list]
            (to_list (bind t ~f))
      ...
      let module Accum = struct
        ...
        let quickcheck_observer =
          Quickcheck.Observer.fixed_point (fun quickcheck_observer ->
            [%quickcheck.observer: [ `Init | `Continue of t * For_testing.Element.t ]])
        ;;
      end
      in
